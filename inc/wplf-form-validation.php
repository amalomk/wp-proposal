 ...
 */
add_filter( 'wplf_validate_proposition', 'wplf_validate_form_exists', 1 );
function wplf_validate_form_exists( $return ) {
  // skip this validation if proposition has already failed
  if ( ! $return->ok ) {
    return $return;
  }
  ...
  if ( ! isset( $_POST['_form_id'] )
    ...
    $return->error = sprintf( __( "Form id %d doesn't exist!", 'wp-proposal' ), intval( $_POST['_form_id'] ) );
  }
 ...
 */
add_filter( 'wplf_validate_proposition', 'wplf_validate_required_empty', 2 );
function wplf_validate_required_empty( $return ) {
  ...
  if ( ! empty( $fields_empty ) ) {
    $return->ok = 0;
    $return->error = __( 'Required fields are missing.', 'wp-proposal' );
...
/**
 * Check that proposition has only fields that are set in form
 */
add_filter( 'wplf_validate_proposition', 'wplf_validate_additional_fields', 3 );
function wplf_validate_additional_fields( $return ) {
  ...
  if ( ! empty( $additional_fields ) ) {
    $return->ok = 0;
    $return->error = __( 'Additional fields are present.', 'wp-proposal' );

...
if ( ! class_exists( 'WPLF_Dynamic_Values' ) ) {
  class WPLF_Dynamic_Values {
    ...
    public function __construct() {
      add_filter( 'WPF_Proposal', array( $this, 'render_form' ), 10, 4 );
    }
    ...
    public static function get_available() {
      $register = function( $value, $callback, $labels = array() ) use ( &$available ) {
        ...
        $available[ $value ] = [
          ...
          'labels' => array_merge([
            'name' => $value,
            'description' => esc_html__( 'No description provided', 'wp-proposal' ),
          ], $labels),
        ];
      ...
      return apply_filters( 'wplf_dynamic_values', array_merge(
        $register('USER_ID', 'get_current_user_id', [
          'name' => esc_html__( 'User ID', 'wp-proposal' ),
          'description' => esc_html__( 'Get current user ID. Prints 0 if user isn\'t logged in.', 'wp-proposal' ),
        ]),
        $register('USER_EMAIL', function () {
          ...
          'name' => esc_html__( 'User email', 'wp-proposal' ),
          'description' => esc_html__( 'Get user email, if it exists.', 'wp-proposal' ),
        ]),
        $register('USER_NAME', function () {
          ...
          'name' => esc_html__( 'User name', 'wp-proposal' ),
          'description' => esc_html__( 'Get user name, if it exists.', 'wp-proposal' ),
        ]),
        $register('TIMESTAMP', function () {
          return date( 'U' );
        }, [
          'name' => esc_html__( 'Timestamp', 'wp-proposal' ),
